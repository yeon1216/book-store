...
public class MyBuyListPanel extends JPanel {
	public JTable table;
	private JTextField inputDealNoTF;
	/**
	 * Create the panel.
	 */
	public MyBuyListPanel(Data data, ArrayList<User> userList, ArrayList<Board> boardList, ArrayList<Book> bookList, ArrayList<Deal> dealList, JPanel cardLayoutPanel) {
		this.setBounds(14, 60, 1246, 613);
		setLayout(null);
		
		JLabel lblNewLabel_5 = new JLabel("구매목록");
		...
		member.showBuyListGUI(userList, table);
		...
		evaluateBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				boolean isSuccess = member.evaluateSellMember(data,inputDealNoTF.getText().trim(),String.valueOf(comboBox.getSelectedItem()));
				if(isSuccess){
					inputDealNoTF.setText("");
					member.showBuyListGUI(userList, table);
				}
